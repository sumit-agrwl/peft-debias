...
bash run_inference.sh $PEFT $MODEL_PATH $IN_DOMAIN_DATA_PATH $IN_DOMAIN_LABEL_COLUMN $IN_DOMAIN_TEXT_COLUMN $PEFT_PATH $DEBIAS_CONFIGURATION "in_domain" $METRIC
...
echo $MODEL_PATH
python scripts/extrinsic_metric.py --dataset $DATASET --output_dir $MODEL_PATH --axis $AXIS